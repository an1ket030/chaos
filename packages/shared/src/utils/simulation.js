...
exports.calculateChemistry = calculateChemistry;
exports.calculateEloChange = calculateEloChange;
// ============================================================
// ATTACK SCORE
// ============================================================
function attackScore(team) {
    const attackers = team.players.filter((p) => ['ST', 'LW', 'RW', 'CAM', 'CM'].includes(p.position));
...
// DEFENSE SCORE
// ============================================================
function defenseScore(team) {
    const defenders = team.players.filter((p) => ['GK', 'LB', 'RB', 'CB', 'CDM'].includes(p.position));
...
// POTM CALCULATION
// ============================================================
function calculatePOTM(events, teamA, teamB) {
    const contributions = {};
...
function simulateMatch(teamA, teamB) {
    ...
    // 15% upset multiplier
    if (strengthA > strengthB && Math.random() < 0.15)
        goalsB += 1;
    if (strengthB > strengthA && Math.random() < 0.15)
        goalsA += 1;
...
function calculateChemistry(players) {
    ...
    for (let i = 0; i < players.length; i++) {
        ...
        if (players[i].position !== players[i].naturalPosition)
            chemistry -= 5;
    }
